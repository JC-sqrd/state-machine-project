class_name OutTransitionSocketUI
extends Control
...
signal pressed(transition_socket_ui : OutTransitionSocketUI)
signal mouse_hovered(transition_socket_ui : OutTransitionSocketUI)
signal mouse_unhovered(transition_socket_ui : OutTransitionSocketUI)

func _ready() -> void:
	mouse_entered.connect(_on_mouse_entered_transition_socket)
	mouse_exited.connect(_on_mouse_exited_transition_socket)

func create_transition_to_state(state : State) -> StateTransition:
	var state_transition : StateTransition = StateTransition.new()
	state_transition.transition_to = state
