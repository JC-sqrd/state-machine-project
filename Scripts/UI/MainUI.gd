...
var selected_transition_socket : TransitionSocketUI
...
func _ready() -> void:
	for child in get_children():
		if child is StateUI:
			...
			child.transition_socket.mouse_hovered.connect(_on_transition_socket_hovered)
			child.transition_socket.mouse_unhovered.connect(_on_transition_socket_unhovered)
			child.transition_socket.pressed.connect(_on_transition_socket_pressed)
...
func _unhandled_input(event: InputEvent) -> void:
	if event is InputEventMouseButton:
		if event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
			...
			var hovered : Control = get_viewport().gui_get_hovered_control()
			if hovered != null and hovered is TransitionSocketUI and current_ui_state != UI_State.SELECT_STATE_TO_TRANSITION:
				current_ui_state = UI_State.SELECT_STATE_TO_TRANSITION
				selected_transition_socket = hovered
				print("Hovered control: " + hovered.name)
				return
			elif hovered != null and hovered is TransitionSocketUI and current_ui_state == UI_State.SELECT_STATE_TO_TRANSITION:
...
func _on_transition_socket_hovered(transition_socket : TransitionSocketUI):
	transition_socket.modulate = Color.GREEN
	pass

func _on_transition_socket_unhovered(transition_socket : TransitionSocketUI):
	transition_socket.modulate = Color.WHITE
	pass

func _on_transition_socket_pressed(transition_socket : TransitionSocketUI):
